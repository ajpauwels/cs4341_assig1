...
public class Genetic {
	...
	public void run() {
		...
		Organism totalBest = null;

		int generations = 1; //keep track of the number of generations

		while (true) {
			if (totalBest != null && totalBest.proximity == 0) {
				System.out.println("[INFO] Solution found!");
				break;
			} else if (endTime - startTime > config.getTimeLimit()*1000) {
				System.out.println("[INFO] Time has run out!");
				break;
			}
			...
			population = newGen;
			generationBest = population.get(0);
			...
			if(totalBest != null){
				if(generationBest.proximity < totalBest.proximity) {
					totalBest = generationBest;
				} else if (generationBest.proximity == totalBest.proximity) {
					if (generationBest.ops.size() < totalBest.ops.size()) {
						totalBest = generationBest;
					}
				}
			}else{
